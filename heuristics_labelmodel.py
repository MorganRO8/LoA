...
def heuristics_labelmodel(args):
    ...
    for task in tasks:
        ...
        csv_file_path = os.path.join(csv_folder, f"{task['name'].replace(' ', '_')}_label_data.csv")
        model_file_path = os.path.join(csv_folder, f"{task['name'].replace(' ', '_')}_label_model.pkl")
        ...
        if os.path.isfile(csv_file_path) and os.path.isfile(model_file_path):
            ...
            # Save the raw output of the labeling heuristics to a CSV file
            df_raw_output = pd.concat([df_train, pd.DataFrame(L_train, columns=[lf.name for lf in lfs])], axis=1)
            df_raw_output.to_csv(csv_file_path, index=False)
            ...
            # Clear the list of labeling functions for the next task
            lfs.clear()
